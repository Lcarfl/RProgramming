pollutantmean <- function(directory, pollutant, id=1:332) {
        ## 'directory' is a character vector of length 1
        ## indicating the location of the CSV files
        
        ##'pollutant'is a character vector of length 1
        ## indicating the name of the pollutant for which we
        ## will calculate the mean; either 'sulfate' or
        ##'nitrate'
        
        ## 'id' is an integer vector indicating the monitor ID
        ## numbers to be used
        
        ## Return the mean of the pollutant across all monitors
        ## list in the 'id' vector (ignoring NA values)
        ## Note: Do not round the result!
        
        if(grepl(directory,getwd()) == F){
                setwd(directory)
        }
        MyFiles <- list.files()[id]
        
        ## Read the pollutant column
        for(i in 1:length(id)) {
                
                MyPollutant <- read.csv(file = MyFiles[i])[[pollutant]]
                if(i>1){ 
                        MyData <- c(MyData,MyPollutant)
                }
                else{
                        MyData <- MyPollutant
                }
        }
        ## Remove the NA values from MyData
        MyData <- MyData[!is.na(MyData)]
        
        MyMean <- mean(MyData)
        setwd("..")
        print(MyMean)
       
}